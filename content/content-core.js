...
// Pre-fetch shadow CSS files so they are ready when the panel/overlay is first created.
// Content scripts cannot use <link> to load external CSS into a shadow root, so the CSS
// text must be injected as a <style> element. Fetching at module load (rather than on
// demand) means the fetch completes during the idle period before the user clicks anything.
const _panelCssReady  = fetch(browser.runtime.getURL('content/panel-shadow.css')).then(r => r.text()).catch(() => '');
const _readerCssReady = fetch(browser.runtime.getURL('content/reader-shadow.css')).then(r => r.text()).catch(() => '');
...
async function getOrCreatePanel() {
  ...
  if (!panel) {
    ...
    style.textContent = await _panelCssReady;
