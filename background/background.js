  ...
  if (message.action === 'findInteresting') {
    getApiSettings()
      .then(({ apiKey, model }) => {
        const elementList = message.elements.map((t, i) => `${i}: ${t}`).join('\n');
        return callAI(
          'You are a strict content relevance filter. Given page elements and user interests, return ONLY elements that directly and specifically match a named interest — not broad tech industry overlap, not tangential context, not coincidental themes. Rules: (1) The match must be to a specific named item in the user\'s interests (e.g. "iPhone", "Debian", "ThinkPad") — vague connections like "tech context" or "relevant industry" are NOT matches. (2) If you cannot name the specific interest that matches, exclude it. (3) When in doubt, exclude it. (4) Never fabricate a connection. Bad example: user likes "iPhone" → SpaceX article matched as "space exploration context" — WRONG, no connection. If no elements genuinely match, return {"matches": []}. Return ONLY valid JSON: an object with a "matches" array of objects with keys "index" (integer) and "reason" (one sentence naming the specific interest matched and how, max 15 words).',
